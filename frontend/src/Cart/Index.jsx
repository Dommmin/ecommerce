...
function Index() {
    ...
    const {data: cartItems, isPending, refetch} = useQuery({
        queryKey: ['cart'],
        queryFn: getCart,
    ...
    if (!cartItems.length) {
        return <div data-test="empty-cart" className="flex justify-center items-center max-w-7xl mx-auto">No products in cart...</div>
    }
        ...
        <div className="dark:text-black">
            ...
            <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
                    ...
                    {cartItems.map((cartItem, index) => (
                        ...
                        >
                            <Link to={'/products/' + cartItem.variant.id}>
                                <img src={cartItem.variant.main_photo} alt={cartItem.variant.product.name}
                                     className="w-full rounded-lg sm:w-40"/>
                            </Link>
                            <div className="sm:ml-4 sm:flex sm:w-full justify-between">
                            <div className="mt-5 sm:mt-0 flex justify-between">
                                    <div>
                                        <h2 className="text-lg font-bold text-gray-900 ">{cartItem.variant.product.name}</h2>
                                        <p className="mt-1 text-xs text-gray-700">{cartItem.option.size.name.toUpperCase()}</p>
                                    </div>
                                    <p className="mt-1 font-bold text-xl text-gray-900 sm:hidden">${cartItem.variant.price}</p>
                                </div>
                                <div
                                    className="mt-4 flex justify-center sm:justify-between sm:mt-0 sm:block sm:space-x-6">
                                <div className="flex items-center border-gray-100">
                                        <button
                                            ...
                                            className={"cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50" + (cartItem.quantity <= 1 ? ' opacity-50 cursor-not-allowed' : '')}>
                                            -
                                        </button>
                                        <span data-test="quantity" className="p-2">{cartItem.quantity}</span>
                                        <button
                                    ...
                                    >Remove</div>
                                    <div
                                        className="pt-6 font-bold text-xl hidden sm:block"
                                    >${cartItem.variant.price}</div>
                                </div>
                                <span
                ...
                <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3 dark:text-black">
                    <div className="mb-2">
                        <input onKeyDown={handleDiscount} type="text" placeholder="Discount code"
